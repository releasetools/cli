...
echo "Adding base::check_deps()" >&2
{
  ...
  echo "# accumulated status is returned. An earlier form ended each line with a subshell"
  ...
  echo "#"
  echo "# Each check now runs *inside* a subshell instead, for two reasons. An 'exit' in a"
  echo "# module would otherwise run in the caller's shell and kill it outright, which even"
  echo "# an 'if' guard could not recover from. And redirecting there sends anything a module"
  echo "# prints to stderr, keeping this function's stdout empty for its callers."
  echo "function base::check_deps() {"
  echo "  local rc=0"
  for m in "${modules[@]}"; do
    echo "  ( $m::_internal_check_deps >&2 ) || { echo \"ERROR: $m::_internal_check_deps\" >&2; rc=1; }"
  done
